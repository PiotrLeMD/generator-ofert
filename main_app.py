    ...
    if total_pacjenci > 0:
        ...
        if st.button(f"➕ Dodaj do Oferty", key=f"btn_{nazwa_uslugi}"):
            if status != "error":
                ...
                if wybrane_opcje:
                    logistyka = f"> **Wybrany zakres:** {', '.join(wybrane_opcje)}\n\n" + logistyka
                    nazwa_w_koszyku += f" ({', '.join(wybrane_opcje)})"
                dodaj_do_koszyka(nazwa_w_koszyku, cena_klienta, cena_per_capita, 0.0, logistyka, marza)
...
# --- DOPASOWANIE DO BUDŻETU ---
    ...
    if wartosc_budzetu > 0:
        ...
        if st.button("🚀 Przeprowadź symulację rentowności", use_container_width=True):
            ...
            # 1. Symulacja Standardowych Usług
            ...
            # 2. Symulacja Zarządzania Stresem (Bez krwi)
            koszt_bazy_stres = pacjenci * 100.0
            if koszt_bazy_stres > 0:
                marza_stres = ((budzet_calkowity - koszt_bazy_stres) / koszt_bazy_stres) * 100 
                item_stres = {"Usługa": "Zarządzanie stresem (Zdalnie/Bez krwi)", "Koszt bazy": koszt_bazy_stres, "Marża": marza_stres, "Czas": 0}
            ...
            if not df_lab.empty:
                ...
                k_pieleg = (pacjenci / 12.5) * 80.0
                k_dojazd_lab = km * 2 * STAWKA_KM * n_zesp_lab
                k_hotel_lab = (dni_lab * KOSZT_NOCLEGU * n_zesp_lab) if (km > 150 or dni_lab > 1) else 0.0
                koszt_ops_lab = k_pieleg + k_dojazd_lab + k_hotel_lab

                laby_zielone = []
                laby_zolte = []

                for index, row in df_lab.iterrows():
                    nazwa_pakietu = row['nazwa']
                    koszt_pakietu_osoba = row['koszt']
                    koszt_bazy_pakietu = koszt_ops_lab + (pacjenci * koszt_pakietu_osoba)
                    
                    if koszt_bazy_pakietu > 0:
                        marza_pakietu = ((budzet_calkowity - koszt_bazy_pakietu) / koszt_bazy_pakietu) * 100
                        if marza_pakietu >= 40: laby_zielone.append(f"{nazwa_pakietu} ({marza_pakietu:.1f}%)")
                        elif 20 <= marza_pakietu < 40: laby_zolte.append(f"{nazwa_pakietu} ({marza_pakietu:.1f}%)")
                
                if laby_zielone:
                    wyniki_zielone.append({"Usługa": "Badania Lab (Polecane)", "Koszt bazy": 0, "Marża": 999, "Czas": dni_lab, "Opis": "Idealnie w budżet wpisują się np.: " + ", ".join(laby_zielone[:4])})
                elif laby_zolte:
                    wyniki_zolte.append({"Usługa": "Badania Lab (Dostępne)", "Koszt bazy": 0, "Marża": 999, "Czas": dni_lab, "Opis": "Możecie wykonać np.: " + ", ".join(laby_zolte[:4])})
                else:
                    wyniki_czerwone.append({"Usługa": "Badania Lab", "Koszt bazy": koszt_ops_lab, "Marża": -100, "Czas": dni_lab, "Opis": "Budżet klienta nie pokryje logistyki i najtańszych pakietów."})

            st.markdown("---")
            
            st.markdown("### 🟢 Zdecydowanie Polecane (Wysoka Rentowność: Marża > 40%)")
            if wyniki_zielone:
                for w in sorted(wyniki_zielone, key=lambda x: x["Marża"], reverse=True):
                    opis = w.get("Opis", f"Szacunkowy koszt bazy: **{w['Koszt bazy']:.2f} PLN**")
                    czas_str = f" | ⏱️ {w['Czas']} dni" if w['Czas'] > 0 else ""
                    if w["Marża"] != 999:
                        st.success(f"**{w['Usługa']}**{czas_str}\n\nSzacowana Marża: **{w['Marża']:.1f}%**\n\n{opis}")
                    else: st.success(f"**{w['Usługa']}**{czas_str}\n\n{opis}")
            else: st.info("Brak usług w tej kategorii dla podanego budżetu.")

            st.markdown("### 🟡 Do negocjacji (Niska Rentowność: Marża 20% - 40%)")
            if wyniki_zolte:
                for w in sorted(wyniki_zolte, key=lambda x: x["Marża"], reverse=True):
                    opis = w.get("Opis", f"Szacunkowy koszt bazy: **{w['Koszt bazy']:.2f} PLN**")
                    czas_str = f" | ⏱️ {w['Czas']} dni" if w['Czas'] > 0 else ""
                    if w["Marża"] != 999:
                        st.warning(f"**{w['Usługa']}**{czas_str}\n\nSzacowana Marża: **{w['Marża']:.1f}%**\n\n{opis}")
                    else: st.warning(f"**{w['Usługa']}**{czas_str}\n\n{opis}")
            else: st.info("Brak usług w tej kategorii.")

            st.markdown("### 🔴 Poza zasięgiem (Odrzucone: Marża < 20% lub strata)")
            if wyniki_czerwone:
                for w in sorted(wyniki_czerwone, key=lambda x: x["Marża"], reverse=True):
                    opis = w.get("Opis", f"Szacunkowy koszt bazy: **{w['Koszt bazy']:.2f} PLN**")
                    czas_str = f" | ⏱️ {w['Czas']} dni" if w['Czas'] > 0 else ""
                    if w["Marża"] not in [-100, 999]:
                        st.error(f"**{w['Usługa']}**{czas_str}\n\nSzacowana Marża: **{w['Marża']:.1f}%**\n\n{opis}")
                    else: st.error(f"**{w['Usługa']}**{czas_str}\n\n{opis}")
            else: st.info("Brak usług w tej kategorii.")
